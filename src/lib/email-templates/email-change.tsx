...
export const EmailChangeEmail = ({ siteName, oldEmail, newEmail, confirmationUrl }: Props) => (
  <Html lang="lt" dir="ltr">
    <Head />
    <Preview>Patvirtinkite el. pašto pakeitimą — {siteName}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Heading style={h1}>Patvirtinkite el. pašto pakeitimą</Heading>
        <Text style={text}>
          Prašėte pakeisti savo el. pašto adresą {siteName}: iš{' '}
          <Link href={`mailto:${oldEmail}`} style={link}>{oldEmail}</Link> į{' '}
